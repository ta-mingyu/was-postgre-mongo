...
description: reports/ 산출물(표준 설정 가이드라인)이 갱신될 때, 기존 harness 표준값·산정 공식·권장 수치가 최신 벤더 문서와 여전히 일치하는지 외부 리서치 기반으로 검증(시효성/freshness 체크). context7 MCP + 벤더 공식 문서 + release notes 로 크로스체크하여 구식/불일치 값 탐지, vendor-research.md 갱신 + 도메인 rules 수정 제안. 가이드 버전업·벤더 신버전 출시·정기 시효성 점검 시 사용.
---

# verify-standards — 표준값 시효성 검증 (외부 리서치)

`update-guide`가 **내부 정합성**(이미 정해진 baseline 대비 70% Ceiling·타임아웃 캐스케이드 점검)을 다룬다면, 이 스킬은 **외부 시효성**(기존 표준값이 최신 벤더 권장값과 일치하는가)을 검증한다. 인프라 권장값은 시간에 따라 변한다(MongoDB 8.0->8.3 Write Concern/defaultMaxTimeMS 변경, GC 기본값 전환, HikariCP 권장값 업데이트 등). reports/ 가 바뀔 때마다 기존 harness 값의 유통기한을 점검한다.
...
| 불변량 | 근거 | 위치 |
| :--- | :--- | :--- |
| 70% Ceiling Rule | 사내 공유 DB 정책 | gotchas.md, 각 rules |
| 방화벽 TCP Established Timeout = 1,800s | 사내망 인프라 고정 | gotchas.md |
| 타임아웃 캐스케이드(27/28/30min) | 방화벽 30min 에서 파생 | gotchas.md |
...
| OS 커널 파라미터 권장값 | somaxconn, file-max | 커널 문서 |

## 절차

### 1. 변경 범위에서 검증 대상 값 추출
갱신되는 reports/ 섹션의 핵심 수치/공식/권장값을 나열한다. 분류 A(불변량)는 제외하고 B(권장값)만 검증 큐에 넣는다.

### 2. 최신 자료 리서칭 (신뢰 계층 순)
각 검증 대상값을 아래 소스로 조사. 상위 계층을 우선한다.
...
- **release notes 의 Breaking Change / Deprecated / 기본값 변경**
- 공식 권장값의 전제 조건(SSD vs HDD, DB 전용 vs 공유 서버 등)

### 3. 비교 테이블 작성
기존 harness 값 vs 최신 권장값 vs 판정.
...
- **일치**: 조치 없음.
- **불일치/구식**: 해당 도메인 rules 수정 제안 + `vendor-research.md` 갱신(출처·날짜 포함).
- **Deprecated/위험**: 즉시 보고 + HITL 분류(표준값 변경은 TA 결정 사항).

### 5. vendor-research.md 갱신 (불일치 발견 시)
- 리서치 결과(값, 출처, 확인일)를 `harness/vendor-research.md` 해당 벤더 섹션에 추가.
- 기존 값은 삭제하지 않고 "변경: 기존값 -> 신값 (근거)" 형태로 이력 보존.

### 6. HITL 가드
표준값 자체(불변량이 아닌 권장값) 변경은 TA 승인 후 반영. 리서치 결과는 **제안**으로만 제시하고 임의 확정 금지(workflow.md HITL 규칙).

## update-guide 와의 관계
- `update-guide` 실행 **직전 또는 1단계와 병행**하여 본 스킬로 시효성을 먼저 점검한다.
