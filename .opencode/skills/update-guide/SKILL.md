---
name: update-guide
description: 표준 설정 가이드라인(WAS/PostgreSQL/MongoDB/PgPool-II) 갱신 절차. 도메인 harness 규칙 로드 -> 산정 공식 baseline 적용 -> 검증 체크리스트 실행 -> 버저닝(-v{N}) + reports/final/ 정본 동기화 + AGENTS.md 링크 갱신 + todo.md 체크 + HITL 가드. 가이드라인 수정·버전업, 표준값 반영, 벤더 리서치 갱신 반영 시 사용.
---

# update-guide — 표준 가이드라인 갱신 절차

이 프로젝트의 산출물(표준 설정 가이드라인)은 버저닝 규칙과 정본 동기화 규칙을 반드시 따른다. 순서를 어기면 `reports/final/`(정본)과 `reports/`(히스토리) 사본이 어긋나고 AGENTS.md 링크가 깨진다.

## 트리거
- WAS/PostgreSQL/MongoDB/PgPool-II 표준값 변경
- 벤더 권장값 리서치 결과 반영
...
### 1. 도메인 harness 로드 (의무)
작업 대상 도메인의 산정 공식 + 검증 체크리스트를 먼저 읽는다.

| 대상 | 로드 파일 |
| :--- | :--- |
| WAS(JVM/Thread/Pool/OS) | `harness/was-rules.md` |
...
### 2. 산정 공식 baseline 적용
- 도메인 rules의 공식/매트릭스를 **기본선**으로 삼는다.
...
- 기본 파일명 예: `was-standard-guide`, `db-standard-guide`.
- N = 기존 최대 버전 + 1.

### 5. 정본 동기화 (reports/final/)
- `reports/final/{was,postgresql,pgpool-ii,mongodb}.md` 해당 정본 사본을 신규 내용으로 갱신.
- `reports/final-standard-guide.md`(통합 확정본)도 해당 섹션 갱신.
- 정본은 항상 최신이어야 한다.

### 6. AGENTS.md 링크 최신화
AGENTS.md 의 "버전 히스토리" 섹션 링크가 신규 버전을 가리키도록 갱신. 이전 버전 파일은 보존(삭제 금지).
