...
| :--- | :--- | :--- | :--- |
| **maxThreads** | `min(CPU_cores * 50, 500)` | 4 Core 기준 **200** (Tomcat 기본값) | 요청을 실제로 처리하는 작업 스레드(Woker Thread)의 최대 개수. 톰캣이 동시에 수행할 수 있는 연산의 상한선을 정의함 |
...
>
> | Boot 3.x (본 가이드) | Boot 2.x (역매핑) | 비고 |
> | :--- | :--- | :--- |
> | `server.tomcat.threads.max` | `server.tomcat.max-threads` | camelCase 또는 하이픈 표기법으로 변경 필요 |
> | `server.tomcat.threads.min-spare` | `server.tomcat.min-spare-threads` | 동일 |
> | `server.tomcat.connection-timeout: 20s` | `server.tomcat.connection-timeout: 20000` | Duration 표기 대신 밀리초(ms) 정수로 변환 |
> | `server.tomcat.keep-alive-timeout: 15s` | `server.tomcat.keep-alive-timeout: 15000` | Spring Boot 2.x에서 해당 키 미지원 시 별도 Filter 또는 `TomcatServletWebServerFactory` Bean으로 프로그래밍 방식 설정 필요 |
> | `server.tomcat.max-keep-alive-requests: 100` | `server.tomcat.max-keep-alive-requests: 100` | Boot 2.x 미지원 시 동일하게 Bean 오버라이드 필요 |
