...
> **적용 범위**: Apache Tomcat 9.x, Spring Boot 내장 Tomcat (3.x / 4.x), IBM WebSphere Liberty 23.x
...
> **Spring Boot 4.x 호환성**: 본 설정값을 그대로 적용. Boot 4.0(2025-11-20 GA)은 내장 Tomcat 10.1 -> 11.0(Servlet 6.1), HikariCP -> 7.0, Java 17+ baseline(Java 25 1급 지원)로 번들 버전이 상승했으나, 위 모든 튜닝 프로퍼티 키와 HikariCP 설정은 3.x와 동일. Boot 4.0 Configuration Changelog에서 튜닝 관련 프로퍼티(`server.tomcat.threads.*`, `max-connections`, `accept-count`, `connection-timeout`, `keep-alive-timeout`, `max-keep-alive-requests`)의 변경/제거 없음을 확인. 제거된 `server.*` 프로퍼티(`server.undertow.*`, `server.error.*`, `server.servlet.encoding.*`)는 본 가이드 튜닝 대상 외.
...
> **Spring Boot 4.x**: 본 규정의 `application.yml` 값을 그대로 적용. 튜닝 프로퍼티 키가 3.x와 동일.
>
...
| server.tomcat.connection-timeout: 20s | 20000 (ms 정수) | 20000 (ms 정수) | Boot 3.x/4.x는 Duration, Boot 2.x는 ms 정수 |
| server.tomcat.keep-alive-timeout: 15s | 15000 (ms, Boot 2.5.0+) | Bean 오버라이드 필요 | 프로퍼티 지원은 **Boot 2.5.0**부터 (기존 "2.3.0" 표기 정정). Boot 4.x는 3.x와 동일 |
| server.tomcat.max-keep-alive-requests: 100 | 100 (Boot 2.4.0+) | Bean 오버라이드 필요 | 프로퍼티 지원은 Boot 2.4.0부터. Boot 4.x는 3.x와 동일 |
