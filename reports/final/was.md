...
> Spring Boot 2.4.0부터 톰캣 스레드 제어 프로퍼티 키가 파괴적 변경(Breaking Change)됨.

| Boot 3.x (본 규정) | Boot 2.4+ | Boot 2.3 이하 | 비고 |
|:---|:---|:---|:---|
| server.tomcat.threads.max | server.tomcat.threads.max | server.tomcat.max-threads | 2.4 미만에서는 max-threads 키 |
| server.tomcat.threads.min-spare | server.tomcat.threads.min-spare | server.tomcat.min-spare-threads | 동일 Breaking Change |
| server.tomcat.connection-timeout: 20s | 20000 (ms 정수) | 20000 (ms 정수) | Boot 2.x는 ms 정수 |
| server.tomcat.keep-alive-timeout: 15s | 15000 (ms, 2.3.0+) | Bean 오버라이드 필요 | Boot 2.3.0부터 프로퍼티 지원 |
| server.tomcat.max-keep-alive-requests: 100 | 100 (2.4.0+) | Bean 오버라이드 필요 | Boot 2.4.0부터 프로퍼티 지원 |
...
### 2.6 IBM WebSphere Liberty 23.x
