...
| `internalQueryMaxBlockingSortMemoryUsageBytes` | `setParameter` 또는 mongod.conf | 예 (런타임, 영구는 conf). 8.0 기본값 100MB. 구이름 `internalQueryExecMaxBlockingSortBytes`는 deprecated alias([SERVER-44053](https://jira.mongodb.org/browse/SERVER-44053)) |
...
`cacheSizeGB`, `maxIncomingConnections`가 아닌 변경은 런타임 명령으로 해결한다. 대표적으로 `internalQueryMaxBlockingSortMemoryUsageBytes`, `defaultMaxTimeMS`, profiling level, `localLogicalSessionTimeoutMinutes` 등. 본 규정은 `internalQueryMaxBlockingSortMemoryUsageBytes`를 8.0 기본값(100MB)으로 적용하므로, 런타임 조정이 필요한 경우(악성 쿼리 격리 등)에만 아래 예시 사용.

```javascript
// mongosh 접속 (Primary에서 실행)

// 1. setParameter -- 즉시 적용 (해당 노드만)
db.adminCommand({ setParameter: 1, internalQueryMaxBlockingSortMemoryUsageBytes: 104857600 })  // 100MB (8.0 기본값)
