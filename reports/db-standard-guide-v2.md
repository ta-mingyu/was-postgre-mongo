# DB 서버 표준 설정 가이드라인 V2

> **버전**: V2 -- 다중 아키텍처 대응 (Standalone / SR / PgPool+SR / Patroni / Replica Set / Sharded Cluster)
> **V1 대비 주요 변경**: PostgreSQL 4종 아키텍처, MongoDB 3종 아키텍처 상세 가이드 추가. 아키텍처 선택 의사결정 매트릭스 신설. 타임아웃 캐스케이드 아키텍처별 분기.
...
```mermaid
flowchart TD
    START([서비스 특성 분석]) --> Q1{트랜잭션/HA 필요?}
    Q1 -->|아니오| Q_DEV{환경?}
    Q_DEV -->|개발/테스트| PG_SA[PostgreSQL Standalone]
    Q_DEV -->|로깅/임시| PG_SA
    Q1 -->|예| Q2{RTO/RPO 요구사항?}

    Q2 -->|RTO 수 시간 허용| Q_SMALL{DBA 수동<br/>페일오버 가능?}
    Q_SMALL -->|예| PG_SR[PostgreSQL SR Only]
    Q_SMALL -->|아니오| Q_POOL{다수 WAS 인스턴스가<br/>DB 공유 또는 읽기 분산 필요?}

    Q2 -->|RTO < 30초| Q_POOL

    Q_POOL -->|아니오| Q_SIMPLE{etcd 운영<br/>부담 수용 가능?}
    Q_POOL -->|예| PG_PGPOOL[PostgreSQL PgPool+SR]

    Q_SIMPLE -->|예| PG_PATRONI[PostgreSQL Patroni+etcd]
    Q_SIMPLE -->|아니오| PG_PGPOOL

    START --> MQ1{MongoDB 데이터 규모?}
    MQ1 -->|개발/테스트| MG_SA[MongoDB Standalone]
    MQ1 -->|프로덕션| MQ2{데이터 > 1TB 또는<br/>쓰기 TPS > 20k?}
    MQ2 -->|아니오| MG_RS[MongoDB Replica Set PSS]
    MQ2 -->|예| MG_SHARD[MongoDB Sharded Cluster]
...
> **[Shard Key 주의]** Shard Key 선택은 사실상 되돌릴 수 없음(MongoDB 4.4 이전) 또는 resharding 비용 매우 큼(MongoDB 5.0+). 카디널리티, 분포 균등성, 쿼리 패턴 종합 분석 후 결정. ESR 규칙(Equality -> Sort -> Range) 준수.
...
| **플랫폼개발 (나이스차저)** | MongoDB | M1/S2/A0, R:W=6:4 | 인스턴스당 **maxPoolSize=20**. **Profiling Level 1 즉시 활성화** | MongoDB만 사용. COLLSCAN 무감지 상태 운영 위험. 정산/결제 서비스는 `primary` 유지 필수 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **PG Standalone** | WAS -> PG | HikariCP maxPoolSize=20 | 없음 | max_conn >= Sum * 1.5 | 소규모 |
| **PG SR Only** | WAS -> PG | HikariCP maxPoolSize=20 | 없음 | max_conn >= Sum * 1.5 | 앱 레벨 읽기 분산 |
...
| **PG Patroni** | WAS -> HAProxy -> PG | HikariCP maxPoolSize=20 | HAProxy(라우팅) + PgBouncer(선택) | max_conn >= Sum * 1.5 | PgBouncer Transaction mode 시 200 직접 연결 -> 2,000+ 클라이언트 수용 가능 |
