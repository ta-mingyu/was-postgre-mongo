...
| 파라미터 | 산정 공식 및 기준 | 비고 |
| :--- | :--- | :--- |
| **maxThreads** | `min(CPU_cores * 50, 500)` | 4 Core 기준 **200** (Tomcat 기본값) |
| **minSpareThreads** | `maxThreads / 8` | 200 스레드 기준 **25** 설정 |
| **acceptCount** | `100` (고정) | 대기 큐 Backlog 표준값 |
| **maxConnections** | `8,192` (고정) | NIO 커넥터 수용 한계 수치 |
| **maxPoolSize (DB)** | `(DB_Cores * 2) + 여유량` | WAS 자원이 아닌 **DB 서버 스펙** 기준 산정 |
...
| **16 GB** | 3 | **3,584m** | **Parallel GC** (`-XX:+UseParallelGC`) | 20 ~ 30 |
...
- G1의 핵심 강점인 Heap 크기 대비 Stop-the-World 시간 제어能力이 실질적인 이점을 제공
...
| :--- | :--- | :--- | :--- |
| **HikariCP maxLifetime** | `1,740,000ms` (29분) | 풀(Pool) 내부 커넥션의 최대 생존 수명 정의 | 하위 모든 DB의 유휴 세션 제한(30분) 및 사내망 방화벽 임계치(30분)보다 1분 먼저 커넥션을 스스로 폐기 및 재생성(Recycle)하여, DB 또는 방화벽이 먼저 연결을 강제 차단함에 따라 발생하는 커넥션 단절 예외(**Connection reset**)를 원천 차단 |
...
HikariCP `maxLifetime`(29분)이 아래 모든 DB의 세션 제한(30분)보다 선행하여 커넥션을 Recycle함으로써
...
| **MongoDB** | `localLogicalSessionTimeoutMinutes` | `30` (30분) | 논리 세션(Logical Session)의 유휴 만료를 제어하여 세션 누적으로 인한 서버 자원 고갈을 방지 |
| **DB2** | `CONNECTIONIDLETIME` (LUW) / `IDTHTOIN` (z/OS) | `30 MINUTE` / `1,800s` | 유휴 세션의 DB 엔진 자원(Memory, Lock) 점유를 해제하여 전체 가용성 확보 |

> **타임아웃 캐스케이드 인과관계**:
>
> ```
> HikariCP maxLifetime (29분)
...
> ```
>
> HikariCP가 모든 하위 계층(DB, 방화벽)보다 **1분 먼저** 커넥션을 폐기 및 재생성(Recycle)함으로써
...
| `agedTimeout` | (미설정) | **1740s** | 커넥션 최대 수명을 PG `idle_session_timeout`(1800s)보다 60초 짧게 설정하여 DB 서버 측 강제 차단 방지 |
...
| `maxLifetime` < PG `idle_session_timeout` | 커넥션 무효화 방지 | DB가 먼저 연결을 종료하여 예외 발생 |
